...
export class BasketController {
  ...
  @Get()
  async getProducts(@Req() req: Request): Promise<BasketsDto[] | undefined> {
  ...
  @UseGuards(AuthGuard('jwt'), PermissionGuard)
  @Delete()
  async deleteProduct(@Req() req: Request): Promise<BasketsDto | undefined> {
    const jwtData = this.jwtTokenService.decode(req.cookies?.jwt);
    if (typeof jwtData === 'object') {
      req.body.userId = jwtData?.id;
      const userProduct = new BasketsDto();
  ...
  @UseGuards(AuthGuard('jwt'), PermissionGuard)
  @Post()
  async addProduct(@Req() req: Request): Promise<BasketsDto | undefined> {
    const jwtData = this.jwtTokenService.decode(req.cookies?.jwt);
    if (typeof jwtData === 'object') {
      req.body.userId = jwtData?.id;
      const userProduct = new BasketsDto();
